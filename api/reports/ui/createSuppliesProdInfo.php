...
try {
 $ID =  $_POST["id"];
$SUPPLYITEMS = GetTransacSupplyItems($ID, $database);
$SUPPLYITEMS = array_map(function($item, $db) {
    ...
    return $item; 
}, $SUPPLYITEMS, array_fill(0, count($SUPPLYITEMS), $database));
} catch (\Throwable $th) {
    echo $th;
}
?>
    ...
    <div class="popup-content">
        <div class="popup-long-container">
            ...
            <div class="popup-body">
                <?php echo CreateTable($SUPPLIESITEMSHEADER, $SUPPLIESITEMSBODY, $SUPPLYITEMS, "ts_product_id", -1, true) ?>
            </div>
        </div>
    </div>
</div>
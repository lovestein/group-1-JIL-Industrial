...
function Refresh() {
  return new Promise((resolve) => {
    Ajax({
      url: "./api/reports/ui/createExpensesTable.php",
      type: "POST",
      success: (table) => {
...
function FilterTable(fromDate, toDate) {
  ...
  return new Promise((resolve) => {
    Ajax({
      url: "./api/reports/ui/filterExpenses.php",
      type: "POST",
      ...data,
      success: (res) => {
        try {
...
function ManageFilters() {
  ...
  const updateData = (data) => {
    if (data) {
      let total = 0;
      if (data && data.length) {
        total = data.map((ob) => parseFloat(ob.total)).reduce((a, b) => a + b);
      }
...
function onButton(id) {
  Ajax({
    url: "./api/reports/ui/createExpensesInfo.php",
    type: "POST",
    data: ToData({ id }),
    success: (popup) => {
      CreateInfoPopup(popup, (parent) => {
        OnButtonManual(parent, (expID) => {
          Ajax({
            url: "./api/reports/ui/createExpensesProdInfo.php",
            type: "POST",
            data: ToData({ id: expID }),
            success: (expPopup) => {
              CreateInfoPopup(expPopup);
            },
          });
        });
      });

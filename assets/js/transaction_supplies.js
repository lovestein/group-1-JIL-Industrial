...
function InsertSupplies(summary) {
  return new Promise((resolve) => {
    Ajax({
      url: "./api/workplace/insertSupplies.php",
      type: "POST",
      data: ToData({
...
function GetSize(tank, type) {
  return new Promise((resolve) => {
    Ajax({
      url: "./api/workplace/getSize.php",
      type: "POST",
...
function SaleManager() {
  ...
  const supplierCombo = pos.querySelector(".supplier");
  ...
  const RemoveItem = (id) => {
    const sale = summary.supplies.filter((s) => s.id === id)[0];

    if (sale) {
      const element = sale.item;

      summary.supplies = summary.supplies.filter((s) => s.id !== id);
      element.remove();
    }
  ...
  const ResetAll = () => {
    ...
    summary.total = 0;
    summary.products = 0;
    summary.supplies.forEach(({ id }) => RemoveItem(id));
  };
  ...
  const InsertSale = (sale) => {
    ...
    summary.supplies.push({ id, item, sale });
    summary.products = summary.supplies.length;
  ...
  const ComputeSales = () => {
    if (summary.supplies.length) {
      const totalamount = summary.supplies
  ...
  const SubmitPOS = () => {
    if (summary.supplies.length) {
      ...
      InsertSupplies(summary).then((res) => {
        if (parseInt(res)) {
  ...
  const getAproducts = (prod) => {
    GetComboAccCategories(prod).then((d) => {
      ...
      if (accessorycombo) {
        ListenToCombo(accessorycombo, applyPrice);
  ...
  ListenToCombo(supplierCombo, (customer) => {
    summary.supplier = customer;
  });
  ...
  createCustomerBtn.addEventListener("click", () => {
    CreateCustomer().then((data) =>
      InsertCustomer(data).then((id) => {
        AddComboItem(supplierCombo, data.cust_name, id);
      })
    );
  });
}
